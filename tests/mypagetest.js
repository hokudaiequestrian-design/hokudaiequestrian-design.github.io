...
const 入口のソース = fs.readFileSync(入口, 'utf8')
  .replace('  始める();',
    '  this.__test = { 一週間を組む, 重なり, 週の見た目, 先の予定, 組み立てる, 節々, 描く, 和風, 足す, 人員表を読む, 送り待ちを直す, 休みの知らせの見た目 };');
...
確かめる('手入れと当番だけの日は知らせない',
  M.重なり([{ 種類: '手入れ' }, { 種類: '当番' }]) === '',
  M.重なり([{ 種類: '手入れ' }, { 種類: '当番' }]));
確かめる('手入れ・当番でも休みと重なれば知らせる',
  M.重なり([{ 種類: '手入れ' }, { 種類: '当番' }, { 種類: '休み' }]) === '手入れと当番と休みが重なっています',
  M.重なり([{ 種類: '手入れ' }, { 種類: '当番' }, { 種類: '休み' }]));
...
// ----- 了承待ちの休みの知らせ（副将のマイページ。2026-09-21） -----
見出し('入口：了承待ちの休みの知らせ');
確かめる('待ちが無ければ何も出さない', M.休みの知らせの見た目({ 待ち: [], 件数: 0 }) === '');
確かめる('読めなかったとき（中身なし）も何も出さない', M.休みの知らせの見た目(null) === '');
{
  const 一件 = M.休みの知らせの見た目({ 件数: 1, 待ち: [
    { id: 'l1', name: '黒木ゆ', kind: '有給休暇', from: 日(2), to: 日(3), days: 2 },
  ] });
  確かめる('件数を数で出す', 一件.indexOf('<b>1件</b>') > 0, 一件);
  確かめる('だれの・何の休みかを出す', 一件.indexOf('黒木ゆ') > 0 && 一件.indexOf('有給休暇') > 0, 一件);
  確かめる('日にちは「〜」でつなぐ', 一件.indexOf('〜') > 0 && 一件.indexOf('2日') > 0, 一件);
  確かめる('休みをまとめる画面への行き先が付く', 一件.indexOf('href="yasumi-admin.html"') > 0, 一件);
  確かめる('読み上げに載せる（role=status）', 一件.indexOf('role="status"') > 0, 一件);
}
{
  const 一日 = M.休みの知らせの見た目({ 件数: 1, 待ち: [
    { id: 'l1', name: '三輪', kind: 'バイト', from: 日(5), to: 日(5), days: 1 },
  ] });
  確かめる('1日だけなら「〜」を付けない', 一日.indexOf('〜') < 0, 一日);
}
{
  const 多い = M.休みの知らせの見た目({ 件数: 7, 待ち: '1234567'.split('').map((n) => (
    { id: 'l' + n, name: '部員' + n, kind: '季節休み', from: 日(2), to: 日(2), days: 1 })) });
  確かめる('件数はぜんぶ数える', 多い.indexOf('<b>7件</b>') > 0, 多い);
  確かめる('並べるのは5件まで', (多い.match(/class="a-row"/g) || []).length === 5, 多い);
  確かめる('あふれたぶんは「ほか○件」', 多い.indexOf('ほか 2件') > 0, 多い);
}
確かめる('名前はそのまま埋め込まない（エスケープする）',
  M.休みの知らせの見た目({ 件数: 1, 待ち: [{ id: 'l1', name: '<script>', kind: 'バイト', from: 日(1), to: 日(1), days: 1 }] })
    .indexOf('<script>') < 0);
