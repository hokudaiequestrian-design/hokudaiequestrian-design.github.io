...
見出し('バイト');
{
  ...
  // 部員はバイトと重なる日でも申し込める。止めずに注意を返す（2026-09-23 ユーザーの指示）
  {
    const r = G.submitLeaveDays(甲.id, '季節休み', [休_先(3), 休_先(4)], '重なりの検査');
    確かめる('バイトと重なる日でも申し込める（注意つき）',
      r.ok && r.count === 1 && r.注意 && r.注意.length === 1 && r.注意[0].date === 休_先(3) && r.注意[0].label === 'フロンテア', JSON.stringify(r));
    const 一覧 = G.yasumiLoadAll(T2).leaves;
    const 待ち = 一覧.filter((l) => l.memberId === 甲.id && l.kind === '季節休み' && l.from === 休_先(3))[0];
    確かめる('申請中として入っている', !!待ち && 待ち.state === '申請中');
    投げるはず('バイトと重なる申し込みは、バイトを外すまで了承できない', () => G.yasumiDecide(T2, 待ち.id, '承認', ''), 'バイト「フロンテア」');
    確かめる('副将の一覧でバイトはバイト先の名前を持つ', 一覧.some((l) => l.kind === 'バイト' && l.label === 'フロンテア' && l.jobId === フロンテア.id));
    G.yasumiDecide(T2, 待ち.id, '却下', '検査');
    const r2 = G.submitLeave(甲.id, '季節休み', 休_先(3), 休_先(3), '期間で');
    確かめる('期間で出すほうにも注意が付く', r2.注意 && r2.注意.length === 1 && r2.注意[0].label === 'フロンテア', JSON.stringify(r2.注意));
    G.yasumiDecide(T2, r2.id, '却下', '検査');
    const d = G.baitoLoadAll(T2, フロンテア.id);
    確かめる('バイトの画面に大会の日が渡る（人員表が無ければ空の表）', d.大会 && typeof d.大会 === 'object', JSON.stringify(d.大会));
  }
...
見出し('カレンダーの「大」を仕事の1文字にする');
{
  ...
  // 公開＝できた人員表を部員に見せる（2026-09-21 ユーザーの指示）。公開するまでは「大」のまま
  人員表に置く('大会', ['ID', '大会名', '開始日', '終了日', '公開'], [['e1', '春の大会', '2026-06-06', '2026-06-07', true]]);
  ...
  */
  ...
  // ----- 公開していない大会は、人員表ができていても「大」のまま（2026-09-21 ユーザーの指示） -----
  {
    人員表に置く('人員表', ['大会ID', '部員ID', '競技ID', '仕事ID', '馬名', '固定'], [
      ['e1', 'j3', 'c1', 'j_un', '', ''],
    ]);
    ...
    確かめる('公開していれば印が付く',
      (G.getCalendarData('2026-06').大会に出る || []).some((x) => (x.印 || []).length));

    人員表に置く('大会', ['ID', '大会名', '開始日', '終了日', '公開'], [['e1', '春の大会', '2026-06-06', '2026-06-07', false]]);
    {
      const d = G.getCalendarData('2026-06');
      確かめる('公開していない大会は印が付かない（画面で「大」）',
        (d.大会に出る || []).every((x) => !(x.印 || []).length), JSON.stringify(d.大会に出る));
      確かめる('公開していなくても大会そのものは出る', (d.大会 || []).length === 1);
    }
    人員表に置く('大会', ['ID', '大会名', '開始日', '終了日', '公開'], [['e1', '春の大会', '2026-06-06', '2026-06-07', true]]);
    人員表に置く('人員表', ['大会ID', '部員ID', '競技ID', '仕事ID', '馬名', '固定'], []);
    人員表に置く('出場', ['ID', '大会ID', '部員ID', '競技ID', '馬名', '馬付き人数'], []);
...
見出し('バイトの自動割り当て');
{
  ...
  */
  {
    ...
    // 片付け（要る人数を消したうえで組み直し、12月の割当も空にする）
    G.baitoSaveSkips(T2, 店.id, []);
  ...
  */
  {
    const 師走 = (n) => '2026-12-' + ('0' + n).slice(-2);
    // みんな同じ回数から始めたいので、まっさらなバイト先を作って見る
    G.baitoSaveJob(T2, { name: 'まっさら' });
    const 店 = G.baitoLoadAll(T2).jobs.filter((j) => j.name === 'まっさら')[0];
    ...
    // 休みの日は避けて配る
    {
      // 同じ日にバイトが入っていると休みを申し込めないので、いったん空にしてから入れる
      G.baitoSaveNeeds(T2, 店.id, '2026-12', {});
      G.baitoGenerate(T2, 店.id, '2026-12', true);
      確かめる('要る人数を空にすると、その月の割当も空になる',
        G.baitoLoadAll(T2, 店.id).割当.length === 0, String(G.baitoLoadAll(T2, 店.id).割当.length));
      ...
      確かめる('休みの日には配らない',
        割.length === 1 && 割[0].memberId !== 休む人.id, JSON.stringify(割.map((x) => x.name)));
