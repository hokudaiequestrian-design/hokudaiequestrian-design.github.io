...
見出し('バイト');
{
  ...
  // バイトと重なる日は申し込めない。「副将に相談」と案内する（2026-09-23 ユーザーの指示）
  {
    投げるはず('バイトと重なる日は申し込めない（日で）', () => G.submitLeaveDays(甲.id, '季節休み', [休_先(3), 休_先(4)], '重なりの検査'), '副将に相談');
    投げるはず('バイトと重なる日は申し込めない（期間で）', () => G.submitLeave(甲.id, '季節休み', 休_先(3), 休_先(3), '期間で'), 'バイトが入っているので申し込めません');
    確かめる('重ならない日なら申し込める', G.submitLeaveDays(甲.id, '季節休み', [休_先(4)], '重ならない').ok);
    const 一覧 = G.yasumiLoadAll(T2).leaves;
    const 待ち = 一覧.filter((l) => l.memberId === 甲.id && l.kind === '季節休み' && l.from === 休_先(4))[0];
